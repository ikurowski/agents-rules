...
describe('runAgentWorkflowCli', () => {
  ...
  test('returns 1 for invalid args and prints problem details', async () => {
    ...
    const printedProblem = JSON.parse(String(errorSpy.mock.calls[0]?.[0])) as {status: number; code: string};

    expect(exitCode).toBe(1);
    expect(logSpy).not.toHaveBeenCalled();
    expect(errorSpy).toHaveBeenCalledOnce();
    expect(printedProblem.status).toBe(400);
    expect(printedProblem.code).toBe('input_invalid_cli');
  });
});
